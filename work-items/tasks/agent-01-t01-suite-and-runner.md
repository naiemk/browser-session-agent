...
- New: `tests/suite/*.task.ts` for definitions, `scripts/run-suite.ts` for the runner
...
4. Build `scripts/run-suite.ts`: runs each task against the agent, enforces `maxSteps`, records per-task outcome, steps used, and token or dollar cost.
...
`scripts/run-suite.ts` runs the full suite headless in CI, exits non-zero only on runner error (not on task failure), and prints success rate, steps per task, and cost per task. `tests/unit/agent-suite.test.ts` is in the `npm test` glob and passes. At least 20 tasks exist with externally authored criteria.
