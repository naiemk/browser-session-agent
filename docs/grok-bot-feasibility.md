...
| --- | --- | --- | --- |
| RESEARCH-01 | Grok Bot stdio MCP via `npx`? | **inferred / unverified for Bot** | Grok Bot docs describe Plugins/connectors and the Agent Computer, not a Bot-side `npx` MCP installer. Magpie parent MVP stays CLI; MCP does not block T01–T03. |
| RESEARCH-02 | Skill install from URL/repo vs marketplace? | **verified (docs) + distribution pattern** | Grok Bot: Settings → Plugins marketplace / packaged skills; enable private skills under Plugins → Yours; save from chat; Teach a task (draft). Type `/` to reference. ([Skills and routines](https://docs.x.ai/grok-bot/skills-routines-and-automations)). There is **no** documented Grok Bot “paste GitHub URL into Plugins to install a skill” path. **T04 copy:** paste the thin bootstrap in `skills/magpie-parent/GROK_BOT.md` once. After that, humans send **only the goal**. The Bot **curl**s canonical `SKILL.md` and executes Magpie (plan, `--json` start, poll, result). Do **not** maintain a forked long procedure only inside the Bot, and do not put Magpie flags in the human prompt. |
...
| RESEARCH-10 | Submit a third-party plugin / packaged skill for Grok Bot? | **verified (docs)** | No public self-serve “submit a plugin/skill” form in official Grok Bot docs. Plugins marketplace / packaged skills appear **curated**; private skills are save-from-chat / Teach a task / Yours. Marketplace tiles (when present) are staff-added — contact xAI for a listing, not a PR path. **Share as template** distributes a whole Bot, not a skill catalog entry. Magpie parent distribution: thin bootstrap paste + git-fetched `SKILL.md`. |
...
skill. There is no documented Grok Bot Plugins “install from GitHub URL” path;
out-of-band listing would require contacting xAI (RESEARCH-10).
