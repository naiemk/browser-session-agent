...
- Cheap reads and forks (D43, D44): `peek` reads a page in a side tab without losing your place, `survey` reads the routes a page advertises without following any, `note_fork` records a word that matched more than one thing, and the suite scores evidence as well as pages so both procedural failures are visible. Next: `work-items/epics/cost-and-audience.md`.
...
- **How often was the approval gate unnecessary?** (D23) Log cases where an authorized commit's preconditions already passed, to judge whether auto-commit is ever safe per goal. Asks on unmatched exploration (authorization none) should be zero.
...
- **Does making the cheap route cheap remove the need for a cost model?** (D44) This build bets that it does, and the bet is falsifiable: if a live run shows the agent still navigating to each item and back with `peek` available, the bet is wrong and measure-one-item-then-commit becomes justified. `roster-cheap-traversal` is the measurement — its budget fits the peeking route and not the navigating one.
- **Does the agent survey before committing to a route, unprompted?** (D44) The card asks for it; whether a model reaches for it under time pressure is a live question. If it does not, the fix is the card rather than more tooling.
- **Is familiarity a liability on well-known sites?** (COST-04) The model can invoke an affordance that existed two years ago without checking the screen, while on an unseen app it cannot hallucinate from priors and is forced to look. Settled by a fixture that mimics a familiar product but differs in one affordance, scored on whether the agent binds to the screen or to its priors.
...
- **A green local test run proved less than we thought.** Two packages the runtime imports were never declared in `package.json`; they resolved locally because npm hoists transitive dependencies to the top of `node_modules`, and failed under `npm ci`, which installs from the lockfile and nests them. The same stale tree hid a dependency bump that had removed `AuthStorage`, so 33 legacy tests were passing locally against a version of `pi-coding-agent` that had not been installed for weeks. The declarations and the 0.84 migration landed on their own; what remains here is the part that stops it recurring — a test that fails on any undeclared import and pins the pi versions together, and `npm run precommit` to run CI's gate in CI's order. When local and CI disagree, suspect the shape of `node_modules` before suspecting the code.
- **`browser.newPage()` silently gives every tab its own cookie jar.** It is documented as creating a page *in a new browser context*, so a second tab opened that way is signed out. A side tab would have shown a stranger's view of the user's own account and we would have reasoned confidently from it. It also meant `openIsolatedTab`'s guarantee held by accident, since ordinary tabs were isolated too. Nothing caught it because every test used one tab. When two things are supposed to differ, test that they differ, not just that each behaves.
- **A planner would have picked the same bad route.** The agent walked a list by navigating to each item and back, and the instinct was to teach it to weigh costs. But it was optimising over the only traversal it had; the missing thing was an action, not judgement. Widen the action set before building machinery to choose among actions. Hence D44.
- **"Needs better planning" hid two failures wanting opposite fixes.** Choosing one meaning of an ambiguous word changes the answer and must be surfaced; choosing a traversal changes only the cost and must never be. Hence D43.
